...
import capitalizeFirstLetter from "../controllers/frontend/capitalizeFirstLetter";
...
const Button = styled.button`
  ...
  border: none;
  cursor: pointer;
  color: var(--translucentColorHard);
  border-top: 1px solid var(--translucentColor);
...
const Container = styled.div`
  ...
  @media (min-width: 950px) {
    ...
    ${({ sideMode }) => {
      if (sideMode)
        return `
     border-left:1px solid var(--translucentColorHard);
      `;
    }}
  }
  ...
  @media (max-width: 950px) {
    ${({ sideMode }) => {
      if (sideMode)
        return `
     border-top:1px solid var(--translucentColorHard);
      `;
    }}
  }
...
function DynamicForm({ setForm, formData, background }) {
